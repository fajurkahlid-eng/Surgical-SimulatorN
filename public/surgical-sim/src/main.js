...
import {
  getSessionContext,
  markSessionStart,
  persistProcedureResult,
  goToAppReports,
  goToAppHome,
} from "./sessionBridge.js";
import { isImmersiveVrSupported } from "./webxrEntry.js";
...
async function main() {
  ...
  const btnStartVr = /** @type {HTMLButtonElement | null} */ (
    document.getElementById("btn-start-vr")
  );
  ...
  const btnGoReports = /** @type {HTMLButtonElement | null} */ (
    document.getElementById("btn-go-reports")
  );
  const btnGoAppHome = /** @type {HTMLButtonElement | null} */ (
    document.getElementById("btn-go-app-home")
  );
  const startInstruction = $("start-screen-instruction");
  const startHint = $("start-screen-hint");

  function applyTraineeNameField(ctx) {
    if (!traineeInput) return;
    traineeInput.value = ctx.traineeName || "";
    traineeInput.readOnly = !!ctx.nameLocked;
    traineeInput.classList.toggle("trainee-name--locked", !!ctx.nameLocked);
    traineeInput.placeholder = ctx.nameLocked ? "" : "Your name";
    traineeInput.setAttribute("aria-readonly", ctx.nameLocked ? "true" : "false");
  }

  function updateStartScreenCopy(ctx) {
    if (startInstruction) {
      startInstruction.textContent = ctx.nameLocked
        ? "Your trainee profile is linked. Begin ward orientation and bedside procedures."
        : "Enter your name to begin ward orientation and bedside procedures.";
    }
    if (startHint) {
      startHint.textContent = ctx.nameLocked
        ? "Click Start to begin."
        : "Press Enter in the name field or click Start";
    }
  }

  function updateReportsButtonsVisibility() {
    const ctx = getSessionContext();
    const exitToApp = Boolean(ctx.exitToApp);
    if (btnGoReports) {
      btnGoReports.classList.toggle("hidden", !exitToApp);
      if (exitToApp) {
        btnGoReports.textContent = "Go to evaluation";
      }
    }
    if (btnGoAppHome) btnGoAppHome.classList.add("hidden");
    if (btnReturn) btnReturn.classList.toggle("hidden", exitToApp);
    if (btnRestart) btnRestart.classList.toggle("hidden", exitToApp);
  }
  ...
  const bridgeEarly = getSessionContext();
  applyTraineeNameField(bridgeEarly);
  updateStartScreenCopy(bridgeEarly);
  if (!bridgeEarly.autoStart && !bridgeEarly.nameLocked) {
    traineeInput?.focus();
  }
  ...
  try {
    ...
    suturing = new SuturingUI(suturingDom, (report) => {
      ...
      persistProcedureResult(report);
      updateReportsButtonsVisibility();
    });
    ...
    void refreshVrStartButton();
  }

  async function refreshVrStartButton() {
    if (!btnStartVr) return;
    const hasApi =
      typeof scene.createDefaultXRExperienceAsync === "function";
    let vrOk = false;
    if (hasApi && world) {
      try {
        vrOk = await isImmersiveVrSupported();
      } catch {
        vrOk = false;
      }
    }
    btnStartVr.disabled = !vrOk || !world;
    btnStartVr.title = vrOk
      ? "Start in immersive VR (WebXR)"
      : !hasApi
        ? "WebXR helpers missing — use a full Babylon.js build"
        : !world
          ? "Loading scene…"
          : "Connect a VR headset and use a WebXR-capable browser (e.g. Chrome / Edge)";
  }
  ...
  function showStartUi() {
    ...
    const ctx = getSessionContext();
    applyTraineeNameField(ctx);
    updateStartScreenCopy(ctx);
    void refreshVrStartButton();
  }

  async function enterVrSession() {
    if (!scene || !world) return;
    if (typeof scene.createDefaultXRExperienceAsync !== "function") {
      alert(
        "WebXR experience helper is not available in this Babylon.js build."
      );
      return;
    }
    try {
      const floorMeshes = Array.isArray(world.buildingMeshes)
        ? world.buildingMeshes.filter((m) => m && m.isEnabled())
        : [];
      if (!xrDefaultExperience) {
        xrDefaultExperience = await scene.createDefaultXRExperienceAsync({
          floorMeshes,
          disableDefaultUI: true,
          uiOptions: { sessionMode: "immersive-vr" },
        });
      }
      const base = xrDefaultExperience?.baseExperience;
      if (!base) {
        alert("WebXR could not initialize on this device.");
        return;
      }
      ...
      alert(
        msg
          ? `Could not start VR: ${msg}`
          : "Could not start VR. Use Start for desktop mode."
      );
    }
  }

  async function beginExplorationWithVr() {
    if (!world || !player) {
      alert(
  ...
  btnGoReports?.addEventListener("click", () => goToAppReports());
  btnGoAppHome?.addEventListener("click", () => goToAppHome());

  const bridgeCtx = getSessionContext();
  if (bridgeCtx.autoStart && world && player) {
    applyTraineeNameField(bridgeCtx);
    updateStartScreenCopy(bridgeCtx);
    beginExploration();
  } else {
    showStartUi();
  }
